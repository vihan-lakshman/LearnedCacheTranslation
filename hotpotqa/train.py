...
###########################
if len(sys.argv) < 2:
    print("Usage: python train_hotpotqa_graduated.py <qwen|mistral>")
    sys.exit(1)
...
SAVE_PATH = f"kv_translators_hotpotqa.pth"

# Configuration
NUM_PROMPTS = 100
MAX_CTX_TOKENS = 1024  # HotpotQA can have long multi-document contexts
TRAIN_STEPS = 5000
BATCH_SIZE = 2
LR = 1e-3
SEED = 42
...
def format_hotpotqa_prompt(example, use_supporting_only=True):
    ...
    Args:
        example: HotpotQA dataset example
        use_supporting_only: If True, only include supporting fact paragraphs
    """
    ...
    return prompt, example['question'], example['answer']
...
def evaluate_translators(model_a, model_b, tokenizer_a, tokenizer_b, translators_k, translators_v, device, eval_dataset):
    """Evaluate translators on held-out HotpotQA examples."""
    print("\n" + "="*80)
    ...
    for i in range(min(NUM_EVAL_SAMPLES, len(eval_dataset))):
        try:
            ...
            context_prompt, question, ground_truth_answer = format_hotpotqa_prompt(example)
...
def main():
    ...
    print(f"Training with HotpotQA dataset")
    ...
    try:
        # Load training data (distractor setting with hard negatives)
        train_dataset = load_dataset("hotpot_qa", "distractor", split="train")
        # Load validation data for evaluation
        eval_dataset = load_dataset("hotpot_qa", "distractor", split="validation")
        
        # Select subsets
        train_dataset = train_dataset.select(range(min(NUM_PROMPTS, len(train_dataset))))
        eval_dataset = eval_dataset.select(range(min(NUM_EVAL_SAMPLES, len(eval_dataset))))
    except Exception as e:
        print(f"Dataset loading failed: {e}")
        print(f"Loading smaller subset...")
        train_dataset = load_dataset("hotpot_qa", "distractor", split="train").select(range(min(NUM_PROMPTS, 100)))
        eval_dataset = load_dataset("hotpot_qa", "distractor", split="validation").select(range(min(NUM_EVAL_SAMPLES, 20)))

    print(f"Loaded {len(train_dataset)} training examples and {len(eval_dataset)} eval examples.")

    print(f"--- Generating Training Data (HotpotQA) ---")
    ...
    for _ in trange(len(train_dataset) // BATCH_SIZE, desc="Generating data"):
        ...
        for _ in range(BATCH_SIZE):
            ...
            prompt, _, _ = format_hotpotqa_prompt(example)
    ...
    sample_prompt, sample_q, sample_a = format_hotpotqa_prompt(train_dataset[0])
    ...
    for step in trange(TRAIN_STEPS, desc="Training"):
        ...
        if (step + 1) % EVAL_EVERY == 0:
            ...
            checkpoint_path = f"kv_translators_hotpotqa_step{step+1}.pth"
    ...
    print("\n" + "="*80)
    print(f"EVALUATION HISTORY (HotpotQA)")
    print("="*80)
