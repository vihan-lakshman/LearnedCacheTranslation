...
###########################
if len(sys.argv) < 3:
    print("Usage: python test_hybrid_translation_hotpotqa.py <qwen|mistral> <cutoff_layer>")
...
LOAD_PATH = "kv_translators_hotpotqa.pth"
MAX_CTX_TOKENS = 1024
...
def format_hotpotqa_prompt(example, use_supporting_only=True):
    """Format a HotpotQA example into prompts for different evaluation modes."""
    ...
    return full_prompt, context_only, question_text, example['answer']
...
def evaluate_baseline_model_b(model_b, tokenizer_b, device, eval_dataset):
    ...
    for i in tqdm(range(min(NUM_EVAL_SAMPLES, len(eval_dataset))), desc="Evaluating Baseline"):
        ...
        full_prompt, context_only, question, ground_truth_answer = format_hotpotqa_prompt(example)
...
def evaluate_hybrid_approach(model_a, model_b, tokenizer_a, tokenizer_b, 
    ...
    import json
    with open(f"hotpotqa_outputs_cutoff{cutoff_layer}.json", "w") as f:
        json.dump(answers, f)
...
def main():
    ...
    print(f"Evaluating Hybrid Translation Approach on HotpotQA Dataset")
    ...
    print("--- Loading HotpotQA Validation Dataset ---")
    try:
        eval_dataset = load_dataset("hotpot_qa", "distractor", split="validation").select(range(NUM_EVAL_SAMPLES))
    except Exception as e:
    ...
    _, dummy_context_only, _, _ = format_hotpotqa_prompt(dummy_example)
    ...
    # Print results
    print("\n" + "="*80)
    print("COMPARATIVE RESULTS (HotpotQA Dataset)")
    print("="*80)
