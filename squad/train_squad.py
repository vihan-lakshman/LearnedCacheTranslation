#!/usr/bin/env python3
"""
Full KV Cache Translation on SQuAD

Translates ALL layers from Model A to Model B using interpolated layer mapping.
This is the comparison baseline for the hybrid approach.

Usage:
    python train_squad_full_translation.py --steps 2000
"""
...
# =============================================================================
# Full Translation SQuAD Trainer
# =============================================================================

class FullTranslationSQuADTrainer:
    """Full translation: translate ALL layers from Model A to Model B."""
    ...
    def setup(self):
        print(f"\n{'='*70}")
        print(f"Full Translation SQuAD Training")
        print(f"{'='*70}")
        ...
        # Create interpolated layer mapping for ALL layers
        # Maps each target layer in B to a source layer in A
        self.layer_mapping = []
        for j in range(self.num_layers_b):
            # Interpolate: which layer in A corresponds to layer j in B?
            src_idx = round(j * (self.num_layers_a - 1) / (self.num_layers_b - 1))
            self.layer_mapping.append(src_idx)
        
        print(f"\n   FULL TRANSLATION CONFIGURATION:")
        print(f"   - Translating ALL {self.num_layers_b} layers")
        print(f"   - Layer mapping (B -> A): {self.layer_mapping[:8]}...{self.layer_mapping[-4:]}")
        ...
        # Create translators for ALL layers
        print("\n3. Creating translators for all layers...")
        with torch.no_grad():
    ...
    def _build_translated_cache(self, prompt: str) -> DynamicCache:
        """Build fully translated cache from Model A."""
        inputs_a = self.tokenizer_a(prompt, return_tensors="pt", padding="max_length",
                                    truncation=True, max_length=self.max_ctx_tokens).to(self.device)
    ...
    def _compute_loss(self, prompts: List[str], answers: List[str]) -> torch.Tensor:
        """Compute NTP loss using fully translated cache."""
        ...
        # Translate ALL layers (with gradients)
        with torch.amp.autocast(device_type=self.device, dtype=torch.float16):
            for i in range(self.num_layers_b):
                src_idx = self.layer_mapping[i]
                src_k = out_a.past_key_values[src_idx][0]
    ...
    def evaluate(self, num_samples: int = None) -> Tuple[float, float]:
        ...
        for i, (ctx, q, gt) in enumerate(tqdm(test_data, desc="Evaluating")):
            ...
            try:
                cache = self._build_translated_cache(prompt)
    ...
    def train(self):
        print(f"\n{'='*70}")
        print(f"Training Full Translation on SQuAD")
        print(f"{'='*70}")
        ...
        for step in trange(self.train_steps, desc="Training"):
            ...
            if (step + 1) % self.eval_every == 0:
                ...
                if f1 > best_f1:
                    best_f1 = f1
                    self.save_checkpoint("squad_full_translation_best.pth")
...
def main():
    ...
    trainer = FullTranslationSQuADTrainer(
    ...
    print(f"  Full translation: ALL {trainer.num_layers_b} layers")
    print(f"  Model A layers: {trainer.num_layers_a}")
    print(f"  Layer mapping: interpolated")
    print(f"\nPerformance:")
    print(f"  Baseline F1: {baseline_f1:.4f}, EM: {baseline_em:.4f}")
    print(f"  Best Full Translation F1: {best_f1:.4f}")
    print(f"  Final Full Translation F1: {final_f1:.4f}, EM: {final_em:.4f}")
