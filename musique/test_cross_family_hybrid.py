...
###########################
if len(sys.argv) < 2:
    print("Usage: python test_cross_family_hybrid.py <cutoff_layer>")
    print("  cutoff_layer: layer at which to switch from translation to recomputation (e.g., 15)")
    print("  Special: cutoff_layer=0 means use native Mistral for all layers")
    print("  Special: cutoff_layer=32 means translate all layers (no recomputation)")
    sys.exit(1)

try:
    CUTOFF_LAYER = int(sys.argv[1])
except:
    print("Argument must be an integer (e.g., 15)")
    sys.exit(1)
...
LOAD_PATH = "kv_translators_cross_family_interpolate_online.pth" # "kv_translators_cross_family_qwen_to_mistral.pth"
MAX_CTX_TOKENS = 1024
...
# Tokenizer alignment strategy
ALIGNMENT_STRATEGY = "pad_shorter"  # Options: "pad_shorter", "truncate_longer"
###########################

if CUTOFF_LAYER > NUM_LAYERS_B:
    print(f"Error: cutoff_layer must be <= {NUM_LAYERS_B}")
    sys.exit(1)
...
def align_kv_cache_sequences(cache_a, cache_b_shape, strategy="pad_shorter"):
    """
    Align KV cache from Model A to match the sequence length of Model B.
    
    Args:
        cache_a: List of (key, value) tuples from Model A
        cache_b_shape: Expected shape from Model B (batch, heads, seq_len, head_dim)
        strategy: "pad_shorter" or "truncate_longer"
    
    Returns:
        Aligned cache matching Model B's sequence length
    """
    target_seq_len = cache_b_shape[2]
    
    aligned_cache = []
    for key, value in cache_a:
        current_seq_len = key.shape[2]
        
        if current_seq_len == target_seq_len:
            aligned_cache.append((key, value))
        elif current_seq_len < target_seq_len and strategy == "pad_shorter":
            # Pad with zeros
            pad_len = target_seq_len - current_seq_len
            key_padded = torch.nn.functional.pad(key, (0, 0, 0, pad_len), value=0.0)
            value_padded = torch.nn.functional.pad(value, (0, 0, 0, pad_len), value=0.0)
            aligned_cache.append((key_padded, value_padded))
        elif current_seq_len > target_seq_len and strategy == "truncate_longer":
            # Truncate
            key_truncated = key[:, :, :target_seq_len, :]
            value_truncated = value[:, :, :target_seq_len, :]
            aligned_cache.append((key_truncated, value_truncated))
        else:
            raise ValueError(f"Cannot align seq_len {current_seq_len} to {target_seq_len} with strategy {strategy}")
    
    return tuple(aligned_cache)
...
def cross_family_hybrid_translate(context_only, tokenizer_a, tokenizer_b, 
                                   model_a, model_b, translators_k, translators_v, 
                                   layer_mapping, cutoff_layer, device, max_length):
    """Cross-family hybrid: Translate early Qwen->Mistral layers, recompute late Mistral layers."""
    
    # Special case: cutoff=0 means use native Mistral for everything
    if cutoff_layer == 0:
    ...
    # First, get the target sequence length from Model B
    inputs_b = tokenizer_b([context_only], return_tensors="pt", truncation=True,
                          max_length=max_length).to(device)
    ...
    target_seq_len = native_cache_b[0][0].shape[2]
    target_shape = native_cache_b[0][0].shape
    del inputs_b, outputs_b
    ...
    # Translate Qwen cache to Mistral format for all layers
    translated_keys = []
    translated_vals = []
    ...
    with torch.no_grad():
        with torch.amp.autocast(device_type=device, dtype=torch.float16, enabled=MIXED_PRECISION):
            for b_layer in range(NUM_LAYERS_B):
                ...
                trans_k = translators_k[b_layer](source_keys[a_layer])
                trans_v = translators_v[b_layer](source_vals[a_layer])
                
                # Align sequence length
                current_seq_len = trans_k.shape[2]
                if current_seq_len < target_seq_len:
                    # Pad with zeros
                    pad_len = target_seq_len - current_seq_len
                    trans_k = torch.nn.functional.pad(trans_k, (0, 0, 0, pad_len), value=0.0)
                    trans_v = torch.nn.functional.pad(trans_v, (0, 0, 0, pad_len), value=0.0)
                elif current_seq_len > target_seq_len:
                    # Truncate
                    trans_k = trans_k[:, :, :target_seq_len, :]
                    trans_v = trans_v[:, :, :target_seq_len, :]
                
                translated_keys.append(trans_k)
                translated_vals.append(trans_v)
    
    # If cutoff equals NUM_LAYERS_B, return full translation
    if cutoff_layer >= NUM_LAYERS_B:
        return tuple(zip(translated_keys, translated_vals))
    
    # Hybrid: translated early layers + native late layers
    hybrid_keys = []
    hybrid_vals = []
...
def evaluate_baseline_mistral(model_b, tokenizer_b, device, eval_dataset):
    """Evaluate Mistral baseline (full text processing, single-pass)."""
    print("\n" + "="*80)
    print(f"BASELINE EVALUATION: Mistral 7B with Full Text (Single-Pass)")
    print("="*80)
...
def evaluate_native_cache_mistral(model_b, tokenizer_b, device, eval_dataset):
    """Evaluate Mistral using native KV cache (two-pass, no translation)."""
    print("\n" + "="*80)
    print(f"NATIVE CACHE EVALUATION: Mistral with Native KV Cache (Two-Pass)")
    print("="*80)
...
def evaluate_cross_family_hybrid(model_a, model_b, tokenizer_a, tokenizer_b, 
                                  translators_k, translators_v, layer_mapping,
                                  cutoff_layer, device, eval_dataset):
    """Evaluate the cross-family hybrid approach."""
    print("\n" + "="*80)
    print(f"CROSS-FAMILY HYBRID EVALUATION: Qwen 1.5B -> Mistral 7B")
    if cutoff_layer == 0:
        print(f"Using native Mistral for ALL layers (cutoff=0)")
    elif cutoff_layer >= NUM_LAYERS_B:
        print(f"Translating ALL layers (full translation, cutoff={cutoff_layer})")
    else:
        print(f"Translating layers 0-{cutoff_layer-1}, Using native Mistral layers {cutoff_layer}-{NUM_LAYERS_B-1}")
    print("="*80)
    ...
    for i in tqdm(range(min(NUM_EVAL_SAMPLES, len(eval_dataset))), desc="Evaluating Cross-Family Hybrid"):
        example = eval_dataset[i]
        full_prompt, context_only, question, ground_truth_answer = format_musique_prompt(example)
        ...
        try:
            ...
            hybrid_cache = cross_family_hybrid_translate(
                context_only, tokenizer_a, tokenizer_b, model_a, model_b,
                translators_k, translators_v, layer_mapping, cutoff_layer, device, MAX_CTX_TOKENS
            )
...
def main():
    ...
    print(f"Cross-Family Hybrid Translation: Qwen 1.5B -> Mistral 7B")
    print(f"  Cutoff layer: {CUTOFF_LAYER}")
    print(f"  Sequence alignment: {ALIGNMENT_STRATEGY}")
    if CUTOFF_LAYER == 0:
        print(f"  Special mode: Using native Mistral for ALL layers")
    elif CUTOFF_LAYER >= NUM_LAYERS_B:
        print(f"  Special mode: Translating ALL layers (full translation)")
    else:
        print(f"  Translated layers: 0-{CUTOFF_LAYER-1} ({CUTOFF_LAYER} layers)")
        print(f"  Native Mistral layers: {CUTOFF_LAYER}-{NUM_LAYERS_B-1} ({NUM_LAYERS_B-CUTOFF_LAYER} layers)")
    print()
    ...
    # Load translators
    print(f"--- Loading Cross-Family Translators from {LOAD_PATH} ---")
    
    checkpoint = torch.load(LOAD_PATH, map_location=DEVICE)
    layer_mapping = checkpoint['layer_mapping']
    
    print(f"Layer mapping loaded. Sample:")
    ...
    dummy_tk = [out_b_dummy.past_key_values[i][0] for i in range(NUM_LAYERS_B)]
    dummy_tv = [out_b_dummy.past_key_values[i][1] for i in range(NUM_LAYERS_B)]
    del inputs_b_dummy, out_b_dummy
    
    translators_k, translators_v = nn.ModuleList(), nn.ModuleList()
    for b_layer in range(NUM_LAYERS_B):
        a_layer = layer_mapping[b_layer]
        sk, sv = dummy_sk[a_layer], dummy_sv[a_layer]
        tk, tv = dummy_tk[b_layer], dummy_tv[b_layer]
    ...
    print("\n[1/3] Evaluating Baseline (Mistral 7B, single-pass)...")
    baseline_f1, baseline_time, baseline_count = evaluate_baseline_mistral(
        model_b, tokenizer_b, DEVICE, eval_dataset
    )
    
    print("\n[2/3] Evaluating Native Cache (Mistral 7B, two-pass with cache)...")
    native_cache_f1, native_cache_time, native_cache_count = evaluate_native_cache_mistral(
        model_b, tokenizer_b, DEVICE, eval_dataset
    )
    
    print("\n[3/3] Evaluating Cross-Family Hybrid...")
    hybrid_f1, hybrid_time, hybrid_count = evaluate_cross_family_hybrid(
        model_a, model_b, tokenizer_a, tokenizer_b,
        translators_k, translators_v, layer_mapping, CUTOFF_LAYER, DEVICE, eval_dataset
    )
    
    # Print results
    print("\n" + "="*80)
    print("COMPARATIVE RESULTS (Cross-Family: Qwen->Mistral)")
    print("="*80)
    print(f"  Evaluation samples: {baseline_count} (with valid answers)\n")
    
    print(f"Baseline (Mistral Single-Pass):        F1 = {baseline_f1:.4f}  ⭐")
    print(f"Native Cache (Mistral Two-Pass):       F1 = {native_cache_f1:.4f}  (diff: {native_cache_f1-baseline_f1:+.4f})")
    print(f"Cross-Family Hybrid (cutoff={CUTOFF_LAYER}):     F1 = {hybrid_f1:.4f}  (diff: {hybrid_f1-baseline_f1:+.4f})")
    print("="*80)
    ...
    if CUTOFF_LAYER > 0 and CUTOFF_LAYER < NUM_LAYERS_B:
        translated_pct = (CUTOFF_LAYER / NUM_LAYERS_B) * 100
        print(f"\n💡 Efficiency: {translated_pct:.1f}% of layers translated from cheaper Qwen 1.5B")
        print(f"   Only {NUM_LAYERS_B-CUTOFF_LAYER} layers require full Mistral 7B computation")
    
    # Performance analysis
    if hybrid_f1 > baseline_f1 - 0.05:
        print(f"\n🎉 Cross-family translation successful! F1 within 5% of baseline")
    elif hybrid_f1 > baseline_f1 - 0.10:
        print(f"\n✅ Promising results - F1 within 10% of baseline")
    else:
        print(f"\n⚠️  Significant degradation - may need different cutoff or more training")
