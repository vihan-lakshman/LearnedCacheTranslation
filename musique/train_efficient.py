...
LR = 1e-3
SEED = 42
TRAINING_BATCH_SIZE = 8  # Reduced from 16
COMPILE_MODEL = False
...
def main():
    ...
    # CRITICAL: Offload models to CPU to free GPU memory for training
    print("\n--- Offloading models to CPU to free GPU memory ---")
    model_a = model_a.cpu()
    model_b = model_b.cpu()
    torch.cuda.empty_cache()
    gc.collect()
    ...
    for step in trange(TRAIN_STEPS, desc="Training"):
        ...
        if (step + 1) % EVAL_EVERY == 0:
            ...
            # Move models back to GPU for evaluation
            print("Moving models to GPU for evaluation...")
