...
###########################
if len(sys.argv) < 2:
    print("Usage: python train_boolq.py <qwen|mistral>")
    sys.exit(1)
...
def compute_ntp_loss(model_b, tokenizer_b, translated_cache, response_text, device):
    ...
    response_ids = tokenizer_b(response_text, return_tensors="pt", padding=False,
...
def main():
    ...
    print(f"Two-Stage Training for BoolQ")
